 ...
 * 実際に請求へ積んだ金額(円)を返す。請求しなかった場合は 0。
 * Why: 呼び出し側(チェックイン画面)が「実際に加算された額」を会員に正しく告知できるようにする
 *      （固定文言だと表示額と実請求額が食い違うため）。
 */
export async function addOverageToNextInvoice(
  member: Pick<GymMember, "stripe_customer_id" | "stripe_subscription_id">,
  gymId: string,
): Promise<number> {
  // Why: 顧客IDが無い / 有効なサブスクが無い場合は請求先が確定しないため超過課金しない。
  //      サブスク無しで invoiceItem を作ると「宙吊りの請求項目」になり、将来の別請求に
  //      予期せず合算される恐れがある（口座振替・解約済への誤課金防止）。
  if (!member.stripe_customer_id || !member.stripe_subscription_id) return 0;
...
export async function createCheckoutSession({
  ...
  // JST基準の「今日」。Vercel実行環境はUTCのため、生の new Date() だと JST 0〜9時台の登録で
  // 日割りの対象月・残日数や trial_end(翌々月1日) が1日/1ヶ月ずれ、誤った日割り課金になる。
  // Date.now()+9h を getUTC* で読むと JST の壁時計になる。
  ...
  // 翌々月1日 00:00(JST) を課金開始アンカーにする。Date.UTC(...) は翌々月1日00:00 UTC なので
  // JST_OFFSET を引いて JST 0時に合わせる（月の選択も JST の nowJst 基準）。
  const trialEndUtcMidnight = Date.UTC(nowJst.getUTCFullYear(), nowJst.getUTCMonth() + 2, 1);
  const trialEnd = Math.floor(trialEndUtcMidnight / 1000) - JST_OFFSET_MS / 1000;
