...
/**
 * このセクションは storageState（認証済みセッション JSON）が用意できた段階で
 * `test.skip` を外して有効化する。
 *
 * セッション JSON の作成方法:
 *   1. `npx playwright codegen http://localhost:3000 --save-storage=auth/free.json`
 *   2. ブラウザでマジックリンクログイン → セッション確立
 *   3. 生成された JSON を e2e/auth/ に保存
 *   4. playwright.config.ts に storageState プロジェクトを追加
 *
 * 必要なセッションファイル:
...
test.describe.skip("認証済み権限マトリックス [TODO: storageState設定後に有効化]", () => {
