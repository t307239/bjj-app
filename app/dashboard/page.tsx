...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const [{ data: profile }, { count: totalCount }] = await Promise.all([
    supabase.from("profiles").select("belt, start_date").eq("id", user.id).single(),
  ...
  if (profile?.start_date) {
    ...
    months = Math.max(0, Math.floor((Date.now() - start.getTime()) / (1000 * 60 * 60 * 24 * 30)));
  }
...
export default async function DashboardPage() {
  ...
  const [
    ...
    supabase.from("training_logs").select("date").eq("user_id", user.id).order("date", { ascending: false }).limit(60),
  ]);

  let streak = 0;
  if (recentLogs && recentLogs.length > 0) {
    const dates = [...new Set(recentLogs.map((l: { date: string }) => l.date))].sort(
      ...
      <main className="max-w-4xl mx-auto px-4 py-6">
        <div className="mb-6">
          ...
          <p className="text-gray-400 text-sm mt-1">
            {streak >= 30 ? `🔥 ${streak}日連続！圧倒的な継続力です。`
              : streak >= 14 ? `💪 ${streak}日連続！素晴らしいペースです。`
              : streak >= 7 ? `⚡ ${streak}日連続！勢いが出てきました！`
              : streak >= 3 ? `🎯 ${streak}日連続！良い習慣が育っています。`
              : streak >= 1 ? "今日も練習頑張ろう！"
              : "今日から新しい練習を記録しよう！"}
          </p>
        </div>
