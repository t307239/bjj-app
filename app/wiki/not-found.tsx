import Link from "next/link";
import { detectServerLocale, makeT } from "@/lib/i18n";

/**
 * #22: BJJ-themed 404 page for the Wiki section.
 * locale-aware: ユーザーの bjj_locale cookie / Accept-Language に従って翻訳。
 */
export default async function WikiNotFound() {
  const locale = await detectServerLocale();
  const t = makeT(locale);
  // 「BJJ Wikiに戻る」先は locale 別。デフォルトは en。
  const wikiHref = `/wiki/${locale}`;

  return (
    <div className="min-h-[100dvh] bg-[#0f172a] text-white flex items-center justify-center px-4">
      <div className="text-center max-w-md">
        <div className="text-7xl mb-6">🥋</div>

        <h1 className="text-3xl font-bold text-white mb-3 tracking-tight">
          {t("wiki.notFoundTitle")}
        </h1>

        <p className="text-slate-400 mb-3 leading-relaxed">
          {t("wiki.notFoundSwept")}
        </p>
        <p className="text-slate-500 text-sm mb-10">
          {t("wiki.notFoundBody")}
        </p>

        <div className="flex flex-col sm:flex-row items-center justify-center gap-3">
          <Link
            href={wikiHref}
            className="inline-flex items-center gap-2 rounded-xl bg-pink-600 hover:bg-pink-500 px-6 py-3 text-sm font-bold text-white transition-colors"
          >
            ← {t("wiki.notFoundBack")}
          </Link>
          <Link
            href="/"
            className="inline-flex items-center gap-2 rounded-xl bg-slate-800/80 hover:bg-slate-700/80 border border-slate-700/60 px-6 py-3 text-sm font-medium text-slate-300 transition-colors"
          >
            {t("wiki.notFoundHome")}
          </Link>
        </div>
      </div>
    </div>
  );
}
