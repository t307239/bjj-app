...
export function useGymDashboard({ initialGym, t }: UseGymDashboardProps) {
  ...
  const printLeaderboard = useCallback(() => {
    ...
    const rows = sorted
      ...
      .join("");
    const title = t("gym.printLeaderboardTitle");
    const html = `<!DOCTYPE html><html><head><meta charset="utf-8"><title>${gym.name} — ${title}</title>
      <style>
      ...
      <p>${monthLabel} · ${t("gym.printLeaderboardGenerated")}</p>
      <table><thead><tr><th>${t("gym.printLeaderboardRank")}</th><th>${t("gym.printLeaderboardAthlete")}</th><th>${t("gym.printLeaderboardBelt")}</th><th style="text-align:center">${t("gym.printLeaderboardSessions")}</th></tr></thead>
