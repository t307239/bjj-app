...
export default function TrainingChart({ userId, isPro = false }: Props) {
        ...
        <div className="blur-sm opacity-40 pointer-events-none select-none">
          ...
          <div className="flex gap-1">
            <div className="flex flex-col gap-0.5 mr-1">
              {Array.from({ length: 7 }, (_, i) => new Intl.DateTimeFormat(locale, { weekday: "narrow" }).format(new Date(2024, 0, 7 + i))).map((d, i) => (
  ...
  const monthLabels: string[] = weeks.map((week, wi) => {
    ...
    const monthAbbrev = new Intl.DateTimeFormat(locale, { month: "short" }).format(dt);
                  ...
                  {week.map((day, di) =>
                    day ? (
                      <div key={di} className="relative group w-3 h-3">
                        ...
                        <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-1 px-2 py-1 bg-zinc-800 border border-white/10 text-white text-xs rounded whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-tooltip shadow-lg">
