...
export default function TechniqueLog({ userId, isPro = false, userBelt = "white" }: Props) {
  ...
  return (
    <div>
      ...
      {dangerConfirmPending && (
        <div className="fixed inset-0 bg-black/60 z-dropdown flex items-center justify-center p-4" role="dialog" aria-modal="true" aria-label={t("technique.dangerConfirmTitle")} onClick={() => setDangerConfirmPending(false)}>
